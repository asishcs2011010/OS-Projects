   ...
   if(check!=0)
     {
        ...
        for(int i=1;i<count_r;i++)
            {
              if(ready[i].in_time+ready[i].p<temp.in_time+temp.p)         
                  {
                     temp=ready[i];
                     return_index=ind_arr[i];
                  }
             else if(ready[i].in_time+ready[i].p==temp.in_time+temp.p) // if periods are same then we compare process id
                    {
