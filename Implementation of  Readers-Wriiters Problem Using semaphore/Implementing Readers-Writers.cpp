...
FILE*file_output=fopen("RW-Log.txt","w");
...
sem_t mutex; // mutex helps in increment of read_count with out inconsistency
sem_t rw_mutex ;// rw_mutex allows mutual exclusibilty in CS
...
{   
  ...
  double start=clock();
    ...
    read_count = read_count + 1;
     ...
     if (read_count == 1) 
       {
         sem_wait(&rw_mutex); // sem_wait allows only one thread and blocks all other threads 
       }
...
fprintf(file_output,"%dth CS Entered at %.1d :%.1d :%.1d by reader thread %d\n" , i+1,actEnterTime->tm_hour,actEnterTime->tm_min,actEnterTime->tm_sec, id);
  ...
  usleep(cs(generator1)*1e3);// simulating real time crtitical cetion
    ...
    sem_wait(&mutex); // sem_wait allows only one thread to go in to ensure correct increment
		read_count=read_count-1;
...
fprintf(file_output,"%dth CS Entered at %.1d :%.1d :%.1d by writer thread %d\n" , i+1,actEnterTime->tm_hour,actEnterTime->tm_min,actEnterTime->tm_sec, id);
   ...
   sem_post(&rw_mutex);// sem_post signals to allow a thread in to CS(basically adds one to rw_mutex semaphore)
...
fprintf(file_output,"%dth CS Exited at %.1d :%.1d :%.1d by writer thread %d \n" , i+1,exitTime->tm_hour,exitTime->tm_min,exitTime->tm_sec, id);
  ...
  // remainder section
    ...
    usleep(rem(generator2)*1e3);// simulating real time remainder section
...
for(int i=0;i<n_w;i++)
{
  pthread_join(writers[i],NULL);  //joining writer threads
}
  
  for(int i=0;i<n_r;i++)
{
  pthread_join(readers[i],NULL);  // joining readers threads
}
